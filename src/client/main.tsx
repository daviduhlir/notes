...
function App() {
  ...
  useEffect(() => {
    api.request<{ user: User }>('/api/auth/me').then((payload) => setSession({ status: 'authenticated', user: payload.user })).catch(() => setSession({ status: 'unauthenticated', user: null })).finally(() => setLoading(false))
  }, [])
  ...
  useEffect(() => {
    if (view.mode === 'edit' && currentNote) {
      setDraft((current) => current && current.dirty ? current : { title: currentNote.title, content: currentNote.content, dirty: false })
    }
  ...
  async function createNote() {
    const payload = await api.request<{ note: Note }>('/api/notes', { method: 'POST', body: JSON.stringify({ title: 'Nová poznámka', content: '' }) })
  ...
  async function saveNote(noteId: string, title: string, content: string) {
    const payload = await api.request<{ note: Note }>(`/api/notes/${noteId}`, { method: 'PUT', body: JSON.stringify({ title, content }) })
  ...
  async function toggleCheckbox(noteId: string, line: number, expected: string, checked: boolean) {
    ...
    try {
      const payload = await api.request<{ note: Note }>(`/api/notes/${noteId}/checkbox`, { method: 'PATCH', body: JSON.stringify({ line, expected, checked }) })
      setNotes((list) => list.map((note) => (note.id === noteId ? payload.note : note)))
    } catch (error: any) {
      if (error?.status === 409) await refreshNotes()
      else setError('Checkbox update failed')
    } finally {
      setPendingCheckboxes((state) => { const next = { ...state }; delete next[key]; return next })
    }
  }
  ...
  return (
    <div className="shell">
      <header className="topbar">
        <div><div className="eyebrow">Personal notes</div><h1>Notes</h1></div>
        <div className="topbar-actions"><button className="primary" onClick={createNote}>New note</button><button className="ghost" onClick={handleLogout}>Logout</button></div>
      </header>
      {error ? <p className="error">{error}</p> : null}
      {view.mode === 'list' && <NotesGrid notes={notes} onOpen={(id) => setView({ mode: 'detail', noteId: id })} onEdit={(id) => setView({ mode: 'edit', noteId: id })} onDelete={(id) => setModal({ type: 'delete-note', noteId: id })} />}
      {view.mode === 'detail' && currentNote ? <NoteDetail note={currentNote} onBack={() => setView({ mode: 'list', noteId: null })} onEdit={() => setView({ mode: 'edit', noteId: currentNote.id })} onDelete={() => setModal({ type: 'delete-note', noteId: currentNote.id })} onToggleCheckbox={toggleCheckbox} pendingCheckboxes={pendingCheckboxes} /> : null}
      {view.mode === 'edit' && currentNote ? <NoteEditor note={currentNote} draft={draft} setDraft={setDraft} onBack={() => setModal({ type: 'discard-changes' })} onSave={saveNote} onDiscard={() => setModal({ type: 'discard-changes' })} /> : null}
      {modal.type === 'delete-note' ? <ConfirmDialog title="Delete note?" confirmLabel="Delete" tone="danger" onCancel={() => setModal({ type: 'none' })} onConfirm={() => deleteNote(modal.noteId)} /> : null}
      {modal.type === 'discard-changes' ? <ConfirmDialog title="Discard changes?" confirmLabel="Discard" tone="neutral" onCancel={() => setModal({ type: 'none' })} onConfirm={() => { if (draft?.dirty) { setDraft(null); setView((current) => current.noteId ? { mode: 'detail', noteId: current.noteId } : current) } else { setView((current) => current.noteId ? { mode: 'detail', noteId: current.noteId } : current) } }} /> : null}
    </div>
  )
}
...
function NoteDetail({ note, onBack, onEdit, onDelete, onToggleCheckbox, pendingCheckboxes }: { note: Note; onBack: () => void; onEdit: () => void; onDelete: () => void; onToggleCheckbox: (noteId: string, line: number, expected: string, checked: boolean) => Promise<void>; pendingCheckboxes: Record<string, boolean> }) {
  ...
  return <section className="detail"><button onClick={onBack}>Back</button><div className="detail-head"><h2>{note.title}</h2><div><button onClick={onEdit}>Edit</button><button onClick={onDelete}>Delete</button></div></div><MarkdownRenderer lines={lines} noteId={note.id} onToggleCheckbox={onToggleCheckbox} pendingCheckboxes={pendingCheckboxes} /></section>
}
...
function MarkdownRenderer({ lines, noteId, onToggleCheckbox, pendingCheckboxes }: { lines: string[]; noteId: string; onToggleCheckbox: (noteId: string, line: number, expected: string, checked: boolean) => Promise<void>; pendingCheckboxes: Record<string, boolean> }) {
  return <div className="markdown">{lines.map((line, index) => {
    ...
    if (task) {
      ...
      return <button key={index} className={`task ${pendingCheckboxes[key] !== undefined ? 'pending' : ''}`} onClick={() => onToggleCheckbox(noteId, index + 1, line, !checked)}><span className={`box ${checked ? 'checked' : ''}`} aria-hidden="true">{checked ? '✓' : ''}</span><span>{task[3]}</span></button>
    }
...
function ConfirmDialog({ title, confirmLabel, tone, onCancel, onConfirm }: { title: string; confirmLabel: string; tone: 'danger' | 'neutral'; onCancel: () => void; onConfirm: () => void }) {
  return <div className="dialog-backdrop"><div className="dialog"><h3>{title}</h3><div className="row"><button className="ghost" onClick={onCancel}>Cancel</button><button className={tone === 'danger' ? 'danger' : 'primary'} onClick={onConfirm}>{confirmLabel}</button></div></div></div>
}
